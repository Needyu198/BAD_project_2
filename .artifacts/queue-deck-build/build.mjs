...
const FINAL_PPTX = path.join(workspaceDir, ".artifacts/output/PawEver_Real_Time_Queue_Proposal_v4.pptx");
...
// 2 Problem
{
  const s=presentation.slides.add(); base(s,2,"Clinic queue problem","The current queue page does not update automatically");
...
// 3 Solution
{
  const s=presentation.slides.add(); base(s,3,"Proposed solution","Staff action triggers an immediate update on every connected owner screen");
  const steps=[
...
// 4 Technology
{
  const s=presentation.slides.add(); base(s,4,"Technology of interest","Socket.IO manages a persistent real-time channel between browser and server");
...
// 5 Update flow
{
  const s=presentation.slides.add(); base(s,5,"Queue update flow","One staff action reaches every connected pet-owner screen");
...
// 6 Architecture
{
  const s=presentation.slides.add(); base(s,6,"System architecture","Users, client portals, clinic services, live queue updates, and persistent storage");
...
// 7 Event design
{
  const s=presentation.slides.add(); base(s,7,"Socket.IO event design","The proposed event carries the queue information needed by the owner screen");
...
// 8 Implementation
{
  const s=presentation.slides.add(); base(s,8,"Implementation plan and runnable proof","The prototype proves live delivery; the proposal adds richer events and department rooms");
  const items=[
...
// 9 Demo
{
  const s=presentation.slides.add(); base(s,9,"Live demonstration and testing","Two browser windows show the same queue changing in real time");
...
const result=await finalizePresentation({
  ...
  receiptPath:path.join(TMP_DIR,"validation-v4.json"),
});
